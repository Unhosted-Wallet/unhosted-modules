...
import { StrategyModule } from "../../typechain";
...
export interface SmartAccountSignedStrategyTransaction {
...
export const strategySignTypedData = async (
  handler: string,
  signer: Signer & TypedDataSigner,
  safe: Contract,
  strategyModule: Contract,
  strategyTx: StrategyTransaction,
  chainId?: BigNumberish
): Promise<SafeSignature> => {
  if (!chainId && !signer.provider)
    throw Error("Provider required to retrieve chainId");
  ...
  return {
    ...
    data: await signer._signTypedData(
      ...
      {
        handler: handler,
        value: strategyTx.value,
        data: strategyTx.data,
        nonce: await strategyModule.getNonce(safe.address),
      }
    ),
  };
};
...
export const buildStrategyTransaction = (template: {
...
export async function buildEcdsaModuleAuthorizedStrategyTx(
  handler: string,
  callData: string,
...
): Promise<SmartAccountSignedStrategyTransaction> {
  const stTx: StrategyTransaction = buildStrategyTransaction({
    value: value,
    gas: 3000000,
    data: callData,
  });

  const { signer, data } = await strategySignTypedData(
    handler,
    smartAccountOwner,
    smartAccount,
    strategyModule,
    stTx,
    await strategyModule.getChainId()
  );

  const transaction: StrategyTransaction = {
    value: stTx.value,
    gas: stTx.gas,
    data: stTx.data,
  };

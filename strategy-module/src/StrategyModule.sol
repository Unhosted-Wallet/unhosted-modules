    ...
    IStrategyModule
{
    ...
    {
        address dev = strategyDevs[strategy];
        if (dev == address(0)) {
    ...
    function executeStrategy(Execution[] calldata executions)
        ...
        returns (uint256 gasUsed, bytes[] memory returnData)
    {
        address dev = strategyDevs[executions[0].target];
        if (dev == address(0)) {
            revert InvalidStrategy();
        }
    ...
    function updateStrategy(address strategy, address dev) public {
        if (strategyDevs[strategy] != msg.sender && strategyDevs[strategy] != address(0)) {
            revert NotAuthorized();
        }
        strategyDevs[strategy] = dev;
    }
